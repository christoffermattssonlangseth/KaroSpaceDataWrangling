...
DATA_DIR = Path("/Users/christoffer/Downloads/bruker-wtx-data")

DATASETS = [
    {
        "slug": "brain",
        "label": "Brain",
        "metadata_path": DATA_DIR / "brain_metadata_file.csv.gz",
        "expr_path": DATA_DIR / "brain_exprMat_file.csv.gz",
    },
    {
        "slug": "breast",
        "label": "Breast",
        "metadata_path": DATA_DIR / "breast_metadata_file.csv.gz",
        "expr_path": DATA_DIR / "breast_exprMat_file.csv.gz",
    },
    {
        "slug": "kidney",
        "label": "Kidney",
        "metadata_path": DATA_DIR / "kidney_metadata_file.csv.gz",
        "expr_path": DATA_DIR / "kidney_exprMat_file.csv.gz",
    },
    {
        "slug": "pancreas",
        "label": "Pancreas",
        "metadata_path": DATA_DIR / "pancreas_metadata_file.csv.gz",
        "expr_path": DATA_DIR / "pancreas_exprMat_file.csv.gz",
    },
    {
        "slug": "skin",
        "label": "Skin",
        "metadata_path": DATA_DIR / "skin_metadata_file.csv.gz",
        "expr_path": DATA_DIR / "skin_exprMat_file.csv.gz",
    },
]
...
INTRO_TEMPLATE = """# __TITLE__ To AnnData
...
SETUP_TEMPLATE = """from __future__ import annotations
...
METADATA_PATH = Path("__METADATA_PATH__")
EXPR_PATH = Path("__EXPR_PATH__")
OUTPUT_DIR = ROOT / "data" / "processed" / "__DATASET_ID__"
OUTPUT_PATH = OUTPUT_DIR / "__DATASET_ID__.h5ad"
...
METADATA_TEMPLATE = """METADATA_COLUMNS = [
    "fov",
    "slide_ID",
    "cell_ID",
    "Run_name",
...
LOAD_TEMPLATE = """def read_expr_gene_columns(expr_path: Path) -> list[str]:
...
HIST_TEMPLATE = """fig, axes = plt.subplots(1, 3, figsize=(15, 4))
...
HELPERS_TEMPLATE = """EXPORT_OBS_COLUMNS = [
    "fov",
    "slide_ID",
    "cell_ID",
    "Run_name",
...
def prepare_metadata_for_export(metadata_df: pd.DataFrame) -> pd.DataFrame:
    ...
    obs["dataset_id"] = pd.Categorical(["__DATASET_ID__"] * len(obs))
    obs["sample_id"] = obs["Run_Tissue_name"].astype("string").fillna("__SAMPLE_FALLBACK__").astype("category")
    obs["section_id"] = (
...
ANALYSIS_INPUT_TEMPLATE = """if ANALYSIS_INPUT_MODE not in {"reload_if_available", "reload_only", "memory_only"}:
...
ANALYSIS_TEMPLATE = """if RUN_ANALYSIS:
    try:
        import scanpy as sc
...
CLUSTER_TEMPLATE = """if RUN_ANALYSIS and analysis_adata is not None:
    cluster_key = "cluster"
    try:
...
def build_notebook(dataset: dict[str, Path | str]) -> dict:
    dataset_id = f"bruker-wtx-{dataset['slug']}"
    mapping = {
        "TITLE": f"Bruker WTX {dataset['label']}",
        "METADATA_PATH": str(dataset["metadata_path"]),
        "EXPR_PATH": str(dataset["expr_path"]),
        "DATASET_ID": dataset_id,
        "SAMPLE_FALLBACK": str(dataset["label"]).lower(),
    }

    cells = [
        md_cell(fill(INTRO_TEMPLATE, mapping)),
        code_cell(fill(SETUP_TEMPLATE, mapping)),
        md_cell(
...
"""
        ...
        code_cell(METADATA_TEMPLATE),
        code_cell(LOAD_TEMPLATE),
        code_cell(SUMMARY_TEMPLATE),
        code_cell(HIST_TEMPLATE),
        md_cell(
...
"""
        ...
        code_cell(fill(HELPERS_TEMPLATE, mapping)),
        code_cell(
...
"""
        ...
        code_cell(ANALYSIS_INPUT_TEMPLATE),
        code_cell(ANALYSIS_TEMPLATE),
        code_cell(CLUSTER_TEMPLATE),
        code_cell(
...
def main() -> None:
    ...
    for dataset in DATASETS:
        notebook_path = NOTEBOOK_DIR / f"bruker-wtx-{dataset['slug']}.ipynb"
        notebook = build_notebook(dataset)
